...
{
    public partial class CompositeHoleShape
    {
        ...
        public CompositeHoleShape(IEnumerable<Shape> shapes)
        {
        ...
        public void Process()
        {
            ...
            CompositeHoleShapeData[] eligibleShapes = shapes.Select(x => new CompositeHoleShapeData(x.points.ToArray())).Where(x => x.IsValidShape).ToArray();
            ...
            CompositeHoleShapeData[] holeShapes = eligibleShapes.Where(x => x.parents.Count % 2 != 0).ToArray();
            foreach (CompositeHoleShapeData holeShape in holeShapes)
            {
                ...
                CompositeHoleShapeData immediateParent = holeShape.parents.OrderByDescending(x => x.parents.Count).First();
                immediateParent.holes.Add(holeShape);
            }

            // Solid shapes have an even number of parents
            CompositeHoleShapeData[] solidShapes = eligibleShapes.Where(x => x.parents.Count % 2 == 0).ToArray();
            foreach (CompositeHoleShapeData solidShape in solidShapes)
            {
                solidShape.ValidateHoles();

            }
            // Create polygons from the solid shapes and their associated hole shapes
            Polygon[] polygons = holeShapes.Select(x => new Polygon(x.polygon.points, x.holes.Select(h => h.polygon.points).ToArray())).ToArray();
